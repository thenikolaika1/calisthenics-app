(()=>{
  ...
  if(original){configs.forEach((cfg,i)=>{const view=i===0?original:original.cloneNode(true);view.id=cfg.id;view.classList.remove('active');const img=view.querySelector('img'),title=view.querySelector('h2'),text=view.querySelector('.future-overlay p');if(img){img.removeAttribute('id');img.src=cfg.image;img.alt=cfg.title;img.loading='eager';img.decoding='sync';img.fetchPriority='high'}if(title){title.removeAttribute('id');title.textContent=cfg.title}if(text){text.removeAttribute('id');text.textContent=cfg.text}if(i>0)original.parentNode.insertBefore(view,original.nextSibling)})}
  const map={muscleup:'muscleupView',onearm:'onearmView',frontlever:'frontleverView',planche:'plancheView'},tabs=[...document.querySelectorAll('.skill-tab')],views=()=>[...document.querySelectorAll('.view')];let switching=false;
