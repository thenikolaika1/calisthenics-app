...
function ensureResults(){
 ...
 if(!view){view=document.createElement('section');view.id='resultsView';view.className='view results-view';view.innerHTML='<div class="results-top"><button id="resultsBack" class="menu-back">‹ Меню</button><div><span class="mini">МОЙ ПРОГРЕСС</span><h2>Результаты</h2><p>Рекорды, графики и статистика тренировок</p></div></div><div id="resultsMount"></div>';main.appendChild(view);document.getElementById('resultsBack').onclick=()=>{training.classList.add('hidden');document.getElementById('menuView').classList.remove('hidden');window.scrollTo(0,0)}}
